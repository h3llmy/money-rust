...
impl CategoryRepository for DieselCategoryRepository {
    async fn find_all(&self, user_id: Uuid, pagination: PaginationQuery) -> Result<(Vec<Category>, u64), String> {
        ...
        let mut count_query = categories::table.filter(categories::user_id.eq(user_id)).into_boxed();
        let mut list_query = categories::table.filter(categories::user_id.eq(user_id)).into_boxed();
